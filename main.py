...
if FLAGS.task_mode == 'texture_synthesis': 
    ...
    #Optimizer = Optimizer(targets, None, None, FLAGS)
elif FLAGS.task_mode == 'style_transfer':
...
with sv.managed_session(config = config) as sess:
    ...
    for step in range(max_iter):
        ...
        if ((step+1) in save_step or (step+1) % FLAGS.save_freq == 0):
        ...
        if ((step + 1) % FLAGS.display_freq) == 0:
            ...
            print("current style image : ",FLAGS.target_dir.split('/')[-1])
            if FLAGS.task_mode == 'style_transfer':
                print("current content image : ",FLAGS.target_dir.split('/')[-1])
            print("progress  image/sec %0.1f  remaining %dm" % (rate, remaining / 60))
            print("global_step", step + 1)
        ...
        if ((step+1) in save_step or (step+1) % FLAGS.save_freq == 0):
            ...
            elif FALGS.task_mode == 'style_transfer':
                tar_name = FLAGS.target_dir.split('/')[-1]
                tar_name = tar_name.split('.')[0]
                content_name = FLAGS.target_dir.split('/')[-1]
                content_name = content_name.split('.')[0]
                im = results["outputs"].reshape([size[1],size[2],3])
                scipy.misc.imsave(FLAGS.output_dir +'%s_%i_%.4e_%.4e_%s_%s.png'%(FLAGS.top_style_layer,step+1,results["style_loss"],results["tv_loss"],tar_name,content_name), im)            
